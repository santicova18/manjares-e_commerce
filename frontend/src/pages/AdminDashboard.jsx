    ...
    <div className="min-h-screen bg-[#f2e8d9]" style={{ fontFamily: "'Lato', sans-serif" }}>
      <nav className="bg-[#2c1810] border-b-4 border-[#c4874a] shadow-[0_4px_20px_rgba(44,24,16,0.4)] px-8 py-4 flex items-center justify-between">
        <div className="flex items-center">
          ...
          <h1 className="text-[#fdfaf5] font-bold text-xl" style={{ fontFamily: "'Playfair Display', serif" }}>
      ...
      <div className="max-w-7xl mx-auto p-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-3xl text-[#2c1810] font-bold" style={{ fontFamily: "'Playfair Display', serif" }}>
            ...
            <div className="bg-[#faf5ee] rounded-3xl p-8 max-w-2xl w-full shadow-[-8px_0_40px_rgba(44,24,16,0.3)] border-l-4 border-[#c4874a]">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[#8b5e3c] text-xs uppercase tracking-widest font-medium block mb-1">Nombre</label>
                    <input type="text" value={formData.nombre} onChange={(e) => setFormData({...formData, nombre: e.target.value})} required className="w-full bg-[#f2e8d9] border-2 border-[#e8c99a] rounded-2xl px-4 py-3 text-[#2c1810] focus:outline-none focus:border-[#c4874a]" />
                  </div>
                  <div>
                    <label className="text-[#8b5e3c] text-xs uppercase tracking-widest font-medium block mb-1">Precio</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input type="text" value={formData.unidad_medida} onChange={(e) => setFormData({...formData, unidad_medida: e.target.value})} placeholder="kg, bulto, unidad..." className="w-full bg-[#f2e8d9] border-2 border-[#e8c99a] rounded-2xl px-4 py-3 text-[#2c1810] focus:outline-none focus:border-[#c4874a]" />
                  </div>
                  <div>
                    <label className="text-[#8b5e3c] text-xs uppercase tracking-widest font-medium block mb-1">Stock</label>
                    <input type="number" value={formData.stock} onChange={(e) => setFormData({...formData, stock: e.target.value})} className="w-full bg-[#f2e8d9] border-2 border-[#e8c99a] rounded-2xl px-4 py-3 text-[#2c1810] focus:outline-none focus:border-[#c4874a]" />
                  </div>
                  <div>
                    <label className="text-[#8b5e3c] text-xs uppercase tracking-widest font-medium block mb-1">Categoría</label>
                    <select value={formData.categoria_id} onChange={(e) => setFormData({...formData, categoria_id: parseInt(e.target.value)})} className="w-full bg-[#f2e8d9] border-2 border-[#e8c99a] rounded-2xl px-4 py-3 text-[#2c1810] focus:outline-none focus:border-[#c4874a]">
                      {categorias.map(cat => (
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  <button type="submit" className="flex-1 bg-[#5c3317] text-[#fdfaf5] py-3 rounded-2xl font-bold hover:bg-[#c4874a] hover:text-[#2c1810] transition shadow-[0_4px_15px_rgba(92,51,23,0.3)]">
                ...
                {productos.map((producto) => (
                  <tr key={producto.id} className="hover:bg-[#f2e8d9] transition">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        {producto.imagen_url ? (
                          <img src={producto.imagen_url} alt={producto.nombre} className="w-12 h-12 rounded-lg object-cover" />
                        ) : (
                          <div className="w-12 h-12 rounded-lg bg-[#e8c99a] flex items-center justify-center text-xl">🌾</div>
                        )}
                        <div>
                    ...
                    <td className="px-6 py-4">
                      <span className="font-bold text-[#5c3317] text-lg" style={{ fontFamily: "'Playfair Display', serif" }}>
                        ${formatPrecio(producto.precio)}
                      </span>
                      <span className="text-[#c4874a] text-sm"> / {producto.unidad_medida}</span>
                    </td>
                    <td className="px-6 py-4 text-[#2c1810]">{producto.stock} unidades</td>
                    <td className="px-6 py-4">
                      <button
                        onClick={() => toggleDisponibilidad(producto)}
                        className={`px-3 py-1 rounded-full text-xs font-medium ${producto.is_disponible ? "bg-[#4a5e3a] text-[#fdfaf5]" : "bg-[#8b5e3c] text-[#fdfaf5]"}`}
                      >
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => handleEdit(producto)} className="px-4 py-2 bg-[#e8c99a] text-[#5c3317] rounded-xl hover:bg-[#c4874a] hover:text-[#2c1810] transition text-sm font-medium">
                          Editar
                        </button>
                        <button onClick={() => handleDelete(producto.id)} className="px-4 py-2 bg-[#8b5e3c] text-[#fdfaf5] rounded-xl hover:bg-[#2c1810] transition text-sm font-medium">
                          Eliminar
                        </button>
                      </div>
                    </td>
